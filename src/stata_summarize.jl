...
function stata_summarize(df::AbstractDataFrame, cols = nothing;
    ...
    for c in colsyms
        ...
        if isempty(v)
            ...
            μ = Statistics.mean(v); σ = length(v) > 1 ? Statistics.std(v) : NaN
            push!(mean_, μ); push!(sd, σ)
            push!(mn, minimum(v)); push!(mx, maximum(v))
            if detail
                # Stata uses biased moments for skewness/kurtosis.
                m2 = Statistics.mean((v .- μ).^2); m3 = Statistics.mean((v .- μ).^3); m4 = Statistics.mean((v .- μ).^4)
    ...
    if detail
        ...
        return DataFrames.DataFrame(Variable = names_, Obs = obs, Mean = mean_,
